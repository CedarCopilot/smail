...
export const checkCalendarTool = createTool({
  id: 'check-calendar',
  description: "Check the user's calendar and return available time slots for scheduling meetings",
...
export const searchPersonTool = createTool({
  ...
  inputSchema: z.object({ query: z.string().describe('Name or email of the person to look up') }),
  ...
  execute: async () => {
    ...
    return {
      ...
      notes: ['Responds quickly before 10am local time', 'Prefers weekday mornings for meetings'],
    };
  },
});
...
export const writeEmailTool = createTool({
  ...
  inputSchema: z.object({
    email: z.string().describe('The fully drafted email content (subject + body)'),
  }),
