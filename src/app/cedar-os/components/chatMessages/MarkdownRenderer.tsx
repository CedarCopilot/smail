    ...
    <Wrapper className={wrapperClassName}>
      <ReactMarkdown
        ...
        components={{
            ...
            return isInline ? (
              ...
              >
                ...
                <pre className="p-4 overflow-x-auto">
                  <code className="text-sm font-mono" style={{ color: '#d4d4d4' }}>
                    {codeString}
                  </code>
                </pre>
              </div>
          ...
          h1: ({ children }) => (
            <h1 className="text-2xl font-bold mt-4 mb-2">{processChildren(children)}</h1>
          ),
          h2: ({ children }) => (
            <h2 className="text-xl font-bold mt-3 mb-2">{processChildren(children)}</h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-lg font-bold mt-2 mb-1">{processChildren(children)}</h3>
          ),
          h4: ({ children }) => (
            <h4 className="text-base font-bold mt-2 mb-1">{processChildren(children)}</h4>
          ),
          h5: ({ children }) => (
            <h5 className="text-sm font-bold mt-1 mb-1">{processChildren(children)}</h5>
          ),
          h6: ({ children }) => (
            <h6 className="text-xs font-bold mt-1 mb-1">{processChildren(children)}</h6>
          ),
          blockquote: ({ children }) => (
            <blockquote
          ...
          strong: ({ children }) => (
            <strong className="font-bold inline">{processChildren(children)}</strong>
          ),
          em: ({ children }) => <em className="italic inline">{processChildren(children)}</em>,
          ul: ({ children }) => (
          ...
          thead: ({ children }) => (
            <thead className="border-b-2" style={{ borderColor: styling.accentColor }}>
              {children}
            </thead>
          ),
          tbody: ({ children }) => (
            <tbody className="divide-y" style={{ borderColor: `${styling.accentColor}30` }}>
              {children}
            </tbody>
          ),
          tr: ({ children }) => (
            <tr className="border-b" style={{ borderColor: `${styling.accentColor}20` }}>
              {children}
            </tr>
          ),
          th: ({ children }) => (
          ...
          td: ({ children }) => (
            <td className="px-4 py-2" style={{ borderRight: `1px solid ${styling.accentColor}20` }}>
              {children}
            </td>
          ),
        }}
