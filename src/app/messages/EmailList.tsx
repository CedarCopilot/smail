...
export function EmailList() {
  ...
  const filteredEmails = useMemo(() => {
    let filtered = emails.filter((email) => {
      ...
      switch (filter.view) {
        case 'inbox':
          return !email.isSent && !email.isDraft && !email.isTrash && !email.isSpam;
        case 'starred':
  ...
  const meetingEmails = filteredEmails.filter((e) => {
    ...
    return includesAny(e.subject, meetingKeywords) || includesAny(e.body, meetingKeywords);
  });
  ...
  const responseEmails = filteredEmails.filter((e) => {
    ...
    const referencesExist = Array.isArray(e.references) && e.references.length > 0;
  ...
  return (
    <div className="flex-1 bg-white dark:bg-gray-900 rounded-lg overflow-hidden">
      ...
      <div className="border-b border-gray-200 dark:border-gray-700 px-4 py-2 flex items-center gap-2">
        <Checkbox checked={allSelected} onCheckedChange={handleSelectAll} className="mr-2" />
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-800">
            ...
            <section className="py-4">
              ...
              {importantEmails.length === 0 ? (
                <p className="px-4 py-6 text-sm text-gray-500">No important emails</p>
              ) : (
            ...
            <section className="py-4">
              ...
              {meetingEmails.length === 0 ? (
                <p className="px-4 py-6 text-sm text-gray-500">No meeting-related emails</p>
              ) : (
        ...
        <div className={`flex items-center gap-3 px-4 ${densityClass}`}>
          ...
          <div className="flex-1 flex items-center gap-4 min-w-0">
            <div className="w-48 flex items-center gap-2">
              ...
              <span className="truncate text-sm">{email.from.name || email.from.email}</span>
            </div>

            <div className="flex-1 flex items-center gap-2 min-w-0">
              <span className="truncate text-sm">{email.subject}</span>
              <span className="text-gray-500 dark:text-gray-500 font-normal"> - </span>
