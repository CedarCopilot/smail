...
export function InlineComposeEmail({ parentEmail }: InlineComposeEmailProps) {
  ...
  const inlineDraft = composeDrafts.find((draft) => draft.id === inlineDraftId && draft.isInline);

  if (!inlineDraftId || !inlineDraft) {
    return (
      <div className="flex items-center justify-center p-8 text-gray-500">Loading compose...</div>
    );
  }
