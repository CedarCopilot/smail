...
export function Sidebar({ isOpen }: SidebarProps) {
  const { filter, setView, emails, labels, createComposeDraft } = useEmailStore();
  ...
  const getEmailCount = (view: EmailView): number => {
    return emails.filter((email) => {
      switch (view) {
        case 'inbox':
          return (
            !email.isSent && !email.isDraft && !email.isTrash && !email.isSpam && !email.isRead
          );
        case 'starred':
                  ...
                  {isOpen && (
                    <>
                      ...
                      {item.count && item.count > 0 && (
                        <span
                          className={`text-xs ${
                            isActive ? 'font-bold' : 'text-gray-500 dark:text-gray-500'
                          }`}
                        >
                          {item.count}
                        </span>
